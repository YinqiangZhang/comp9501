...
class RewardConfigsTrialEvalCallback(EvalCallback):
        ...
        if not len(LOGGER.handlers):
            ...
            if log_path is not None:
                file_handler = logging.FileHandler(
                    os.path.join(save_path, "console_copy.txt")
                )
                if verbose == 2:
    ...
    def _on_step(self):
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            def evaluate_policy_configs(
                ...
                for _ in range(n_eval_episodes):
                    ...
                    while not done:
                        ...
                        episode_is_friction_violation.append(
                            info[-1]["is_friction_violation"]
                        )
            ...
            LOGGER.info(
                "Evaluating at learning time step: {}".format(self.num_timesteps)
            )
            LOGGER.info("Cost mean: {:.2f}, std: {:.2f}".format(mean_cost, std_cost))
            LOGGER.info(
                "Length mean: {:.2f}, std: {:.2f}".format(mean_length, std_length)
            )
...
class MultiEnvsEvalCallback(EventCallback):
    ...
    def __init__(
        ...
        # Set basics
        super(MultiEnvsEvalCallback, self).__init__(
            ...
            def evaluate_policy_multi_envs(
                ...
                if is_recurrent:
                    zero_completed_obs = np.zeros(
                        (model.n_envs,) + model.observation_space.shape
                    )
                ...
                while True:
                    ...
                    actions, states = model.predict(
                        obs, state=states, deterministic=deterministic
                    )
                    ...
                    for i in range(env.num_envs):
                        if dones[i]:
                            ...
                            LOGGER.debug(
                                "Collected {} evaluation episodes".format(
                                    len(episode_rewards)
                                )
                            )
                        else:
...
class SaveVecNormalizeCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            LOGGER.info(
                "Saving vectorized and normalized environment wrapper to {}".format(
                    self.save_path
                )
            )
