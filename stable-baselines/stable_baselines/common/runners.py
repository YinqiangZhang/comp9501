...
class AbstractEnvRunner(ABC):
    def __init__(self, *, env: Union[gym.Env, VecEnv], model: 'BaseRLModel', n_steps: int):
        ...
        if isinstance(self.env.action_space, MultiDiscrete):
            ...
            # get reset action mask for reset situation
            self.action_masks = list()
            for idx in range(self.n_envs):
                reset_action_mask = self.env.get_attr('action_mask', idx)[0]
                self.action_masks.append(flatten_action_mask(self.env.action_space, reset_action_mask))
            ...
            self.action_masks = list()
            for idx in range(self.n_envs):
                reset_action_mask = self.env.get_attr('action_mask', 0)[0]
                self.action_masks.append(flatten_action_mask(self.env.action_space, reset_action_mask))
        else:
            self.action_mask_shape = (self.n_envs * (n_steps + 1),)
            self.action_masks = [None for _ in range(self.n_envs)]
            # TODO: generate first initial action mask from the environment 
